...
export default function Home() {
  ...
  const [selectedEntity, setSelectedEntity] = useState<TreemapEntity | null>(
    null,
  );
  ...
  const parts = useMemo(() => {
    ...
    budgetData
      .filter((b) => b.row_type === "part_total")
      .forEach((b) => partTotals.set(b.Part, b));
    ...
    const partMap = new Map<
      string,
      {
        ...
        sections: Map<
          string,
          { sectionName: string; entities: TreemapEntity[] }
        >;
      }
    >();
    const entityRows = budgetData.filter((b) => b.row_type === "entity_total");
    const sectionRows = budgetData.filter(
      (b) => b.row_type === "section_total",
    );
    ...
    partOrder.forEach((partKey) => {
      ...
      partData.sections.forEach((sectionData, sectionKey) => {
        const sectionTotal = sectionData.entities.reduce(
          (sum, e) => sum + e.budget,
          0,
        );
      ...
      if (partTotal > 0) {
        ...
        const varianceVs2025 =
          approved2025 > 0
            ? ((revised2026 - approved2025) / approved2025) * 100
            : 0;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container-padding mx-auto max-w-[1400px] py-8">
        ...
        {/* Budget Overview */}
        {!loading && grandTotal && <BudgetOverview grandTotal={grandTotal} />}
        ...
        <section className="mb-16">
          ...
          {loading ? (
            ...
            <BudgetTreemap parts={parts} onEntityClick={setSelectedEntity} />
          )}
        </section>
